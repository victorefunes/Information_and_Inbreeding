...
# ── Method NV: naive pool matching (Arpino & Cannas 2016, baseline) ──────────
...
# ── Arpino & Cannas (2016): PSM methods for clustered data ───────────────────
# Cluster variable: controller_number (AI company / stud).
# NV (naive pool matching) is match_data_1 above.
#
# Methods added:
#   W  – single-level PS, within-cluster matching (exact = controller_number)
#   FE – fixed-effects logit PS, pool matching
#   RE – random-effects logit PS, pool matching
#   PW – preferential within-cluster matching (novel method in the paper)

# Individual-level covariate formula without parent-company dummies.
# FE and RE absorb cluster-level variation through fixed/random effects on
# controller_number, making the broader company dummies redundant there.
formula_indiv <- superstar ~
  pta_milk + pta_fat_lb + pta_protein_lb + pta_pl +
  pta_dpr + pta_hcr + pta_ccr + pta_liv + pta_type + pta_gest_length +
  pta_heifer_liv + pta_efcalving + pta_mastitis + pta_metritis +
  pta_strength + pta_rear_legs_rear + pta_foot_leg_score +
  pta_teat_rear_place + pta_rump_angle + pta_thurl_width + pta_rear_legs_side +
  pta_foot_angle + pta_fore_udder + pta_rear_udder_height +
  pta_rear_udder_width + pta_udder_cleft + pta_udder_depth +
  pta_teat_front_place + pta_teat_length + pta_milk_fever +
  pta_stature + pta_dairy_form + pta_body_depth + sire_ce + daughter_ce +
  TR + TC + TD + TL + TM + TY + TP + BY + BR + D + RC + CD +
  HH1C + HH2C + HH5C + HH1T + HH2T + HH4T + HH5T + HH6T + HHRC + HCDT + HCDC +
  A1A1 + A1A2 + A2A2 + AA + AB + BB

# ── Method W: same single-level PS as NV; matching within controller_number ──
match_data_W <- matchit(
  superstar ~ pta_milk+pta_fat_lb+pta_protein_lb+pta_pl+
    pta_dpr+pta_hcr+pta_ccr+pta_liv+pta_type+pta_gest_length+
    pta_heifer_liv+pta_efcalving+pta_mastitis+pta_metritis+
    pta_strength+pta_rear_legs_rear+pta_foot_leg_score+
    pta_teat_rear_place+pta_rump_angle+
    ...
    jlg+other+stg+ssires+semex,
  data = data_1, method = "nearest", replace = TRUE,
  exact = ~controller_number,
  verbose = TRUE, ratio = ratio, distance = "glm",
  link = "logit", discard = "none"
)

# ── Method FE: fixed-effects logit PS, pool matching ─────────────────────────
# Adds a dummy for each controller_number, absorbing all company-level variation.
fe_glm <- glm(
  update(formula_indiv, . ~ . + factor(controller_number)),
  data = data_1, family = binomial("logit")
)
data_1$ps_fe <- predict(fe_glm, type = "response")

match_data_FE <- matchit(formula_indiv, data = data_1,
                         method = "nearest", replace = TRUE,
                         distance = data_1$ps_fe,
                         verbose = TRUE, ratio = ratio, discard = "none")

# ── Method RE: random-effects logit PS, pool matching ────────────────────────
# Models unobserved company-level heterogeneity via a random intercept.
re_glmer <- glmer(
  update(formula_indiv, . ~ . + (1 | controller_number)),
  data = data_1, family = binomial("logit"),
  control = glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5))
)
data_1$ps_re <- predict(re_glmer, type = "response")

match_data_RE <- matchit(formula_indiv, data = data_1,
                         method = "nearest", replace = TRUE,
                         distance = data_1$ps_re,
                         verbose = TRUE, ratio = ratio, discard = "none")

# ── Method PW: preferential within-cluster matching ──────────────────────────
# Uses same NV propensity scores. For each treated bull, first searches for the
# nearest control within the same controller_number; if none falls within the
# caliper, searches across all companies (pool fallback).
pref_within_match <- function(data, ps, treat_col, cluster_col, caliper, k = 1) {
  treated <- which(data[[treat_col]] == 1)
  control <- which(data[[treat_col]] == 0)
  ps_c    <- ps[control]
  cl_c    <- data[[cluster_col]][control]

  mm <- matrix(NA_character_, nrow = length(treated), ncol = k,
               dimnames = list(as.character(treated), as.character(seq_len(k))))

  for (ii in seq_along(treated)) {
    i    <- treated[ii]
    t_ps <- ps[i]
    t_cl <- data[[cluster_col]][i]
    diff <- abs(ps_c - t_ps)

    # Step 1: within-cluster candidates
    w_ok <- cl_c == t_cl & diff <= caliper
    if (sum(w_ok) >= k) {
      mm[ii, ] <- as.character(control[w_ok][order(diff[w_ok])[seq_len(k)]])
      next
    }
    # Step 2: cross-cluster fallback
    a_ok <- diff <= caliper
    if (sum(a_ok) >= k) {
      mm[ii, ] <- as.character(control[a_ok][order(diff[a_ok])[seq_len(k)]])
    }
  }
  mm
}

ps_nv   <- match_data_1$distance
caliper <- 0.2 * sd(ps_nv)

pw_mm <- pref_within_match(data_1, ps_nv, "superstar", "controller_number",
                            caliper, k = ratio)

# Embed the PW match matrix in a copy of the NV matchit object so that
# match.data() and get_matches() work identically downstream.
match_data_PW              <- match_data_1
match_data_PW$match.matrix <- pw_mm
